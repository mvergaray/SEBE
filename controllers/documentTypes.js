...
DocumentTypeCtrl.getObjects = (req, res) => {
  var docType = {
        id: req.query.id,
        name: req.query.name
      },
...
DocumentTypeCtrl.saveObjects = (req, res) => {
  ...
  dbQuery('INSERT INTO DOCUMENT_TYPE SET ?;', docType, function (err, result) {
    if (err) {
      printLog(err);
      res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
      return;
...
DocumentTypeCtrl.getObject = (req, res) => {
  ...
  dbQuery(dataQuery, [id], function (err, result) {
    ...
    if (result && result.length) {
      ...
      res.status(500).send({code: 500, msg: 'Document type not found'});
    }
  });
};

DocumentTypeCtrl.updateObject = (req, res) => {
  var id = req.params.id,
      docTypeBP = req.body,
      docType = {};
  ...
  dbQuery('UPDATE DOCUMENT_TYPE SET ? WHERE ID = ?;', [docType, id],
    function (err) {
      if (err) {
        printLog(err);
        res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
...
DocumentTypeCtrl.deleteObject = (req, res) => {
  var id = req.params.id,
      docType = {
        // Set default values
        updated_at: new Date(),
  ...
  dbQuery('UPDATE DOCUMENT_TYPE SET ? WHERE ID = ?;', [docType, id], function (err) {
    if (err) {
      printLog(err);
      res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
      return;
